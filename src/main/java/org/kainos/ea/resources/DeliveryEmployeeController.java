...
public class DeliveryEmployeeController {
    ...
    @Path("/deliveryEmployee/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getDeliveryEmployee(@PathParam("id") int id)
    {
        try {
            return Response.ok(deliveryService.getDeliveryEmployee(id)).build();
        } catch (FailedToGetDeliveryEmployee e) {
            System.err.println(e.getMessage());
            return Response.serverError().build();
        } catch (DeliveryEmployeeDoesNotExistException e) {
            System.err.println(e.getMessage());
